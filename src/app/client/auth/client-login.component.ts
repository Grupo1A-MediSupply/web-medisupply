  ...
  <div class="login-page">
    ...
    <div class="login-right-section">
      <div class="login-form-container">
        <h1 class="login-title">Inicia sesión - Cliente</h1>
        ...
        <form [formGroup]="_form" (ngSubmit)="login()" class="login-form">
          ...
          <div class="forgot-password">
            <a routerLink="/client/change-password" class="forgot-link">Cambiar contraseña</a>
          </div>
        ...
        <div class="login-info">
          <p>Portal exclusivo para clientes institucionales de MediSupply.</p>
          <p><strong>Usuario de prueba:</strong> cliente</p>
          <div class="role-switch">
            <a routerLink="/vendor/login" class="role-link">¿Eres vendedor? Accede aquí</a>
          </div>
        </div>
      </div>
    </div>
        ...
        next: (response) => {
          ...
          try {
            ...
            if (!hasExplicitError) {
              ...
              console.log('✅ Intentando navegar a /client/mfa');
              
              // Usar solo el router de Angular (sin fallbacks que recargan la página)
              console.log('✅ Navegando usando router.navigate');
              ...
              this.router.navigate(['/client/mfa'], { replaceUrl: false }).then(
                (success) => {
                  ...
                  if (!success) {
                    ...
                    this.router.navigateByUrl('/client/mfa', { replaceUrl: false }).catch((err) => {
            ...
            sessionStorage.setItem('role', 'client');
            sessionStorage.setItem('userType', 'client');
            console.log('✅ Intentando navegar a /client/mfa desde catch');
            this.router.navigateByUrl('/client/mfa').catch((navError) => {
              console.error('❌ Error en navegación desde catch:', navError);
              // Intentar con navigate como fallback
              this.router.navigate(['/client/mfa']).catch((err) => {
